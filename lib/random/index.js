...
function getMeasurementTimestamps(sensorType, years = 1) {
  const today = new Date();
  const pastYears = addYears(today, -years);
  const type = getMeasurementType(sensorType);

  const measurementTimestamp = chance.date({ min: pastYears, max: today });
...
function getMeasurement(minLatitude, maxLatitude, minLongitude, maxLongitude, minAltitude, maxAltitude, years) {
  const id = getId();
  ...
  const timestamps = getMeasurementTimestamps(sensor.sensor_type, years);
